...
const NavBar = ({ db, highlighted }) => {
  ...
  return (
    <navbar>
      ...
      {db && (db.piPhone || db.piLocation || db.piIp || db.piDevice || db.piPayment) &&<><div>|</div>
      ...
      {db && ((db.activity && db.activity.find(({app}) => app.includes('linkedin'))) || db.connectedCompanies || db.connectedPositions || db.connections || db.linkedinProfile) &&<><div>|</div>
        <div className={(highlighted === 'linkedin' ? 'highlighted-nav' : '') + ' nav-item'} onClick={() => navigate('/linkedin')}>LinkedIn</div></>}
      {db && (db.spotify || db.spotifyProfile) &&<><div>|</div>
        <div className={(highlighted === 'spotify' ? 'highlighted-nav' : '') + ' nav-item'} onClick={() => navigate('/spotify')}>Spotify</div></>}
    </navbar>
  )
}
